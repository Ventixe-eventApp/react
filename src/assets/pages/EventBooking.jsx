...
const EventBooking = () => {
  ...
  const navigate = useNavigate();
    ...
    eventId: id,            
    userId: "",              
    ticketQuatity: parseInt(data.ticketQuantity), 
...
<div className='hero-booking ' style={{backgroundImage: `url(${Hero})` }}>
 ...
 <span>Please select the number of tickets you'd like to book (maximum 10 per person).
   ...
   <div className="form-group">
     ...
     <div className="form-group">
      <label htmlFor="ticketQuantity">Tickets</label>
        <select id="ticketQuantity" {...register('ticketQuantity', { required: 'Ticket Quantity is required' })}
    className={errors.ticketQuantity ? 'input-error' : ''}>
    <option value="">Select quantity</option>
    {[...Array(10)].map((_, i) => (
      <option key={i + 1} value={i + 1}>{i + 1}</option> ))}</select>
         {errors.ticketQuantity && <p className="validation-error"><i className="bi bi-exclamation-octagon"></i>{errors.ticketQuantity.message}</p>}
      </div>
